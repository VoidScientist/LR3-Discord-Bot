...
async function timeExecution(func, parameter) {
    ...
    return [result, (end-start).toFixed(TIME_PRECISION)];
...
async function checkTestCases() {
    ...
    for (let i = 0; i < tests.length; i++) {
        ...
        const checkType = test.hasOwnProperty("checkType") ? test.checkType : "value";

        const [result, duration] = await timeExecution(test.func, test.parameter);
    
        checkingTypes[checkType](result, test, i + 1);
    
        console.log(`[${test.func.name}] Test ${i+1} passed in ${duration} ms.`);
